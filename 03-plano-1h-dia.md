# Plano de estudo - 1h por dia

Este plano foi feito para estudar no máximo 1h por dia, de segunda a sexta, com revisão e testes no sábado.

## Regra da semana

- Segunda a sexta: 1h por dia.
- Sábado: 1h para revisão, questões e caderno de erros.
- Domingo: descanso ou recuperação leve, sem obrigação.

## Modelo de 1h diária

Use sempre o mesmo formato:

1. 10 min: flashcards e revisão curta do dia anterior.
2. 35 min: aula, PDF ou leitura ativa do tópico.
3. 15 min: questões, resumo curto ou criação de novos flashcards.

Se o conteúdo do dia for muito grande, não aumente o tempo. Pare em 1h e continue no próximo dia.

## Modelo do sábado

1. 15 min: revisar flashcards difíceis.
2. 30 min: resolver questões do conteúdo da semana.
3. 10 min: corrigir e registrar erros.
4. 5 min: escolher os 3 pontos fracos para a semana seguinte.

Meta mínima do sábado:

- 20 questões quando a semana tiver conteúdo técnico.
- 10 questões quando a semana tiver Português/Inglês ou revisão pesada.
- 5 novos flashcards a partir dos erros.

## Arquivos para usar

- Dashboard: [[00-dashboard-petrobras]]
- Roteiro de aulas: [[02-roteiro-aulas]]
- Flashcards visíveis no Obsidian: [[flashcards-petrobras]]
- Flashcards para Anki: [[flashcards-petrobras-base.tsv]]
- Testes de sábado: [[05-testes-sabado]]

## Ciclo de 24 semanas

### Semana 1 - Base de Português e Inglês

- [ ] Segunda: Português - interpretação, inferência, ideia central e tipos de texto.
- [ ] Terça: Português - gêneros textuais, coesão e mecanismos referenciais.
- [ ] Quarta: Português - ortografia, classes de palavras e tempos verbais.
- [ ] Quinta: Português - pontuação, concordância, regência, crase e colocação pronominal.
- [ ] Sexta: Inglês - leitura técnica, cognatos, conectores e gramática para interpretação.
- [ ] Sábado: teste S01 em [[05-testes-sabado]].

### Semana 2 - Engenharia de Software I

- [ ] Segunda: modelos de ciclo de vida de software.
- [ ] Terça: metodologias de desenvolvimento e processo iterativo/incremental.
- [ ] Quarta: arquitetura de software e projeto de software.
- [ ] Quinta: processos e práticas de desenvolvimento.
- [ ] Sexta: revisão comparativa entre cascata, incremental, iterativo e ágil.
- [ ] Sábado: teste S02 em [[05-testes-sabado]].

### Semana 3 - Engenharia de Software II

- [ ] Segunda: práticas ágeis, Scrum no contexto de desenvolvimento e ALM.
- [ ] Terça: BDD, TDD e critérios de aceitação.
- [ ] Quarta: integração contínua, CI/CD e Git.
- [ ] Quinta: DevOps, automação e fluxo de entrega.
- [ ] Sexta: BPMN e gerenciamento de processos.
- [ ] Sábado: teste S03 em [[05-testes-sabado]].

### Semana 4 - Requisitos e Experiência do Usuário

- [ ] Segunda: elicitação e gerenciamento de requisitos.
- [ ] Terça: histórias de usuário e critérios de aceitação.
- [ ] Quarta: Lean UX, MVP e prototipação.
- [ ] Quinta: design thinking e projeto centrado no usuário.
- [ ] Sexta: storytelling e personas.
- [ ] Sábado: teste S04 em [[05-testes-sabado]].

### Semana 5 - Arquitetura de Aplicações I

- [ ] Segunda: MVC.
- [ ] Terça: sistemas de N camadas.
- [ ] Quarta: SOA e ESB.
- [ ] Quinta: microsserviços.
- [ ] Sexta: arquitetura orientada a eventos.
- [ ] Sábado: teste S05 em [[05-testes-sabado]].

### Semana 6 - Arquitetura de Aplicações II

- [ ] Segunda: arquitetura cloud native.
- [ ] Terça: conteinerização de aplicações.
- [ ] Quarta: mediação de APIs e gateways.
- [ ] Quinta: REST, JSON e padrões de APIs.
- [ ] Sexta: SOAP, XML, XSLT, UDDI, WSDL, RMI e XMLHttpRequest.
- [ ] Sábado: teste S06 em [[05-testes-sabado]].

### Semana 7 - Arquitetura de Aplicações III

- [ ] Segunda: design patterns.
- [ ] Terça: anti-patterns e padrões corporativos.
- [ ] Quarta: componentização e interoperabilidade.
- [ ] Quinta: persistência, frameworks de persistência e ORM.
- [ ] Sexta: mensageria, publish-subscribe, streaming e busca em dados não estruturados.
- [ ] Sábado: teste S07 em [[05-testes-sabado]].

### Semana 8 - Linguagens de Programação

- [ ] Segunda: orientação a objetos, coleções e tipos genéricos.
- [ ] Terça: threads, escalonamento, sincronização e deadlocks.
- [ ] Quarta: garbage collector, exceções, anotações e profiling.
- [ ] Quinta: HTML5 e CSS3.
- [ ] Sexta: JavaScript, Python 3 e .NET Core.
- [ ] Sábado: teste S08 em [[05-testes-sabado]].

### Semana 9 - Qualidade de Software

- [ ] Segunda: garantia da qualidade de software.
- [ ] Terça: controle de versão e Git.
- [ ] Quarta: testes unitários, integração, funcionais, aceitação, desempenho, carga e vulnerabilidade.
- [ ] Quinta: caixa-branca, caixa-preta, regressão, testes não funcionais e automação de testes.
- [ ] Sexta: refatoração, débito técnico, métricas de código e code smells.
- [ ] Sábado: teste S09 em [[05-testes-sabado]].

### Semana 10 - Auditoria, Estruturas de Dados I

- [ ] Segunda: auditoria de sistemas.
- [ ] Terça: tipos básicos de dados e tipos abstratos de dados.
- [ ] Quarta: listas, filas, pilhas, vetores e matrizes.
- [ ] Quinta: árvores, heap e generalizações.
- [ ] Sexta: passagem por endereço, referência e valor; recursão.
- [ ] Sábado: teste S10 em [[05-testes-sabado]].

### Semana 11 - Estruturas de Dados II e Algoritmos

- [ ] Segunda: algoritmos de pesquisa.
- [ ] Terça: algoritmos de ordenação.
- [ ] Quarta: complexidade de algoritmos e Big O.
- [ ] Quinta: caminho mínimo e noções de grafos.
- [ ] Sexta: revisão de estruturas, árvores AVL, B e B+.
- [ ] Sábado: teste S11 em [[05-testes-sabado]].

### Semana 12 - Arquitetura de Dados I

- [ ] Segunda: modelagem conceitual, lógica e física.
- [ ] Terça: criação e alteração de modelos lógico e físico.
- [ ] Quarta: modelo relacional e DER.
- [ ] Quinta: normalização.
- [ ] Sexta: integridade referencial.
- [ ] Sábado: teste S12 em [[05-testes-sabado]].

### Semana 13 - Arquitetura de Dados II

- [ ] Segunda: SQL.
- [ ] Terça: DDL e DML.
- [ ] Quarta: SGBD, ACID, transações e isolamento.
- [ ] Quinta: independência de dados, performance e banco em memória.
- [ ] Sexta: NoSQL, metadados e engenharia reversa.
- [ ] Sábado: teste S13 em [[05-testes-sabado]].

### Semana 14 - Dados Analíticos, BI e Big Data

- [ ] Segunda: modelagem dimensional.
- [ ] Terça: integração de dados, ETL e transferência de arquivos.
- [ ] Quarta: OLAP, data warehouse e bases multidimensionais.
- [ ] Quinta: relatórios, dashboards e geração de insights.
- [ ] Sexta: data lakes, Big Data e bancos de documentos/grafos.
- [ ] Sábado: teste S14 em [[05-testes-sabado]].

### Semana 15 - Computação em Nuvem

- [ ] Segunda: conceitos de nuvem e benefícios.
- [ ] Terça: alta disponibilidade, escalabilidade, elasticidade e recuperação de desastres.
- [ ] Quarta: regiões, zonas, subscrições, grupos de gestão e recursos.
- [ ] Quinta: identidade, privacidade, conformidade e segurança em nuvem.
- [ ] Sexta: custos, IoT, Infrastructure as Code e automação.
- [ ] Sábado: teste S15 em [[05-testes-sabado]].

### Semana 16 - Produtos, Dados e Planilhas

- [ ] Segunda: Scrum.
- [ ] Terça: Kanban.
- [ ] Quarta: SAFe e gestão de portfólio.
- [ ] Quinta: dado, informação, conhecimento, inteligência; dados estruturados e não estruturados.
- [ ] Sexta: manipulação de dados em planilhas.
- [ ] Sábado: teste S16 em [[05-testes-sabado]].

### Semana 17 - Infraestrutura Computacional e Redes

- [ ] Segunda: processamento paralelo e distribuído.
- [ ] Terça: HPC, filas e processos.
- [ ] Quarta: virtualização de computação, armazenamento e rede.
- [ ] Quinta: TCP/IP, HTTP, HTTPS, FTP e SMTP.
- [ ] Sexta: LDAP, SSL, SAML e OAuth.
- [ ] Sábado: teste S17 em [[05-testes-sabado]].

### Semana 18 - Segurança da Informação I

- [ ] Segunda: segurança física e lógica.
- [ ] Terça: operação de segurança e gestão de vulnerabilidades.
- [ ] Quarta: firewall, proxy, IDS/IPS, DLP, CASB, SIEM, antivírus, EDR e WAF.
- [ ] Quinta: softwares maliciosos, ransomware, vírus, worms, spywares e rootkits.
- [ ] Sexta: monitoração e backup.
- [ ] Sábado: teste S18 em [[05-testes-sabado]].

### Semana 19 - Segurança da Informação II

- [ ] Segunda: ataques DDoS, SQL Injection, XSS, CSRF e Path Traversal.
- [ ] Terça: desenvolvimento seguro.
- [ ] Quarta: SAST, DAST e IAST.
- [ ] Quinta: VPN e MDM.
- [ ] Sexta: IAM, SSO, MFA, RBAC e ABAC.
- [ ] Sábado: teste S19 em [[05-testes-sabado]].

### Semana 20 - Segurança da Informação III

- [ ] Segunda: gerenciamento de resposta a incidente, NIST SP 800-61.
- [ ] Terça: threat intelligence.
- [ ] Quarta: threat hunting.
- [ ] Quinta: testes de penetração.
- [ ] Sexta: STRIDE e MITRE ATT&CK.
- [ ] Sábado: teste S20 em [[05-testes-sabado]].

### Semana 21 - Riscos, Continuidade e Criptografia

- [ ] Segunda: gestão de riscos, ISO 31000.
- [ ] Terça: gestão de continuidade de negócios, ISO 22301.
- [ ] Quarta: SOX, políticas de segurança e classificação da informação.
- [ ] Quinta: ISO 27002, criptografia, certificado digital e assinatura digital.
- [ ] Sexta: segurança em nuvem e segurança em IoT.
- [ ] Sábado: teste S21 em [[05-testes-sabado]].

### Semana 22 - Governança de TI

- [ ] Segunda: modelos de governança de TI.
- [ ] Terça: administração de serviços de TI.
- [ ] Quarta: ITIL v4.
- [ ] Quinta: viabilidade técnica e econômica; ERP.
- [ ] Sexta: governança de dados e DAMA-DMBoK.
- [ ] Sábado: teste S22 em [[05-testes-sabado]].

### Semana 23 - Revisão Integrada I

- [ ] Segunda: Português e Inglês.
- [ ] Terça: Engenharia de Software e Requisitos/UX.
- [ ] Quarta: Arquitetura de Aplicações.
- [ ] Quinta: Linguagens de Programação.
- [ ] Sexta: Qualidade de Software e Estruturas de Dados.
- [ ] Sábado: teste S23 em [[05-testes-sabado]].

### Semana 24 - Revisão Integrada II

- [ ] Segunda: Arquitetura de Dados e BI.
- [ ] Terça: Computação em Nuvem, Produtos e Análise de Dados.
- [ ] Quarta: Infraestrutura e Redes.
- [ ] Quinta: Segurança da Informação.
- [ ] Sexta: Governança de TI e caderno de erros.
- [ ] Sábado: teste S24 em [[05-testes-sabado]].

## Como concluir um tópico

Marque o dia como concluído apenas se você fez:

- teoria ou aula;
- pelo menos 5 questões ou 5 flashcards;
- anotação de erro ou resumo curto;
- revisão rápida dos cards antigos.

