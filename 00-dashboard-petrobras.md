# Dashboard Petrobras

Use esta pasta como vault do Obsidian:

`/storage/projects/concurso-ti/petrobras`

## Arquivos principais

- [[COMECE-AQUI]]
- [[plano-estudos-petrobras-analista-sistemas]]
- [[01-rotina-obsidian]]
- [[flashcards-petrobras-base.tsv]]

## Fluxo diario

1. Abrir o topico da semana no plano.
2. Estudar teoria ou aula por 50 min.
3. Resolver questoes por 50 min.
4. Atualizar checklist, caderno de erros e flashcards por 50 min.
5. Marcar a proxima revisao: R1, R2, R3 ou R4.

## Controle rapido

| Item | Status |
| --- | --- |
| Semana atual | 1 |
| Topico principal | Portugues, Ingles, Engenharia de Software basica |
| Revisoes atrasadas | 0 |
| Cards novos hoje | 0 |
| Questoes hoje | 0 |
| Acertos hoje | 0 |

## Links uteis

- Plano completo: [[plano-estudos-petrobras-analista-sistemas]]
- Rotina no Obsidian: [[01-rotina-obsidian]]
- Template de sessao: [[templates/sessao-estudo]]
- Template de topico: [[templates/topico-edital]]
- Importacao Notion: [[notion-import/README]]
